...
export function OrderCard({ order, className = '' }: OrderCardProps) {
  const statusConfig = OrderStatusMap[order.state]
  ...
  return (
    ...
    >
      ...
      {order.state === ORDER_STATUS.PENDING_CONFIRMATION && (
      ...
      {order.state === ORDER_STATUS.PENDING_QUOTE_CONFIRMATION && (
        <div className="mt-4">
      ...
      {(order.state === ORDER_STATUS.PENDING_CONFIRMATION ||
        order.state === ORDER_STATUS.PENDING_SCHEDULE ||
        order.state === ORDER_STATUS.PENDING_QUOTE_CONFIRMATION ||
        order.state === ORDER_STATUS.MODIFICATION_REQUEST ||
